...
class Report:
    ...
    def generate_image(self, data, prof):
        ...
        vacancy_rate_by_city = data[5]
        width = 0.4
        x_nums1 = np.arange(len(salary_lvl_by_year.keys()))
        x_list1_1 = x_nums1 - width / 2
        x_list1_2 = x_nums1 + width / 2

        self.ax1.bar(x_list1_1, salary_lvl_by_year.values(), width, label='Средняя з/п')
        self.ax1.bar(x_list1_2, salary_lvl_by_year_for_prof.values(), width, label=f'з/п {prof}')
        self.ax1.set_xticks(x_nums1, salary_lvl_by_year.keys(), rotation='vertical')
        self.ax1.tick_params(axis='both', labelsize=8)
        self.ax1.legend(fontsize=8)
        self.ax1.grid(True, axis='y')

        x_nums2 = np.arange(len(salary_lvl_by_year.keys()))
        x_list2_1 = x_nums2 - width / 2
        x_list2_2 = x_nums2 + width / 2

        self.ax2.bar(x_list2_1, count_vac_by_year.values(), width, label='Количество вакансий')
        self.ax2.bar(x_list2_2, count_vac_by_year_for_prof.values(), width, label=f'Количество вакансий\n{prof}')
        self.ax2.set_xticks(x_nums1, count_vac_by_year.keys(), rotation='vertical')
